...
export function registerCapabilityAgentPipelineSettledHandler(handler: CapabilityAgentPipelineSettledHandler): Cleanup {
...
export async function dispatchCapabilityAgentPipelineSettled(
  ...
  const outcomes = await Promise.allSettled([...agentPipelineSettledHandlers].map((handler) => handler(event)));
